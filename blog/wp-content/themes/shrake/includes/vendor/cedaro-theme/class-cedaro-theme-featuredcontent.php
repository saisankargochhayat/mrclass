...
class Cedaro_Theme_FeaturedContent {
	...
	public function customize_register( $wp_customize ) {
		$wp_customize->add_section( 'featured_content', array(
			'title'       => esc_html__( 'Featured Content', 'shrake' ),
			'description' => sprintf( esc_html__( 'Your theme supports up to %d posts in its featured content area.', 'shrake' ), $this->max_posts ),
			'priority'    => 130,
		) );
	...
	public function print_templates() {
		...
		<script type="text/html" id="tmpl-cedaro-theme-featured-content-button-add">
			<button class="button button-secondary alignright"><?php esc_html_e( 'Add Posts', 'shrake' ); ?></button>
		</script>
		...
		<script type="text/html" id="tmpl-cedaro-theme-featured-content-post">
			...
			<div class="ctfc-post-inside">
				<div class="ctfc-post-actions">
					<a class="ctfc-post-remove js-remove"><?php esc_html_e( 'Delete', 'shrake' ); ?></a> |
					<a class="js-close"><?php esc_html_e( 'Close', 'shrake' ); ?></a>
				</div>
			</div>
		</script>
		...
		<script type="text/html" id="tmpl-cedaro-theme-featured-content-modal">
			<div class="ctfc-modal-head find-box-head">
				<?php esc_html_e( 'Find Posts', 'shrake' ); ?>
			...
			<div class="ctfc-modal-inside find-box-inside">
				<div class="ctfc-modal-search find-box-search">
					...
					<input type="button" value="<?php esc_attr_e( 'Search', 'shrake' ); ?>" class="button ctfc-modal-search-button" />
					<div class="clear"></div>
				</div>
	...
	public function ajax_find_posts() {
		...
		if ( ! $posts ) {
			wp_send_json_error( esc_html__( 'No items found.', 'shrake' ) );
		}
	...
	protected function get_found_posts_html( $posts ) {
		...
		$html = sprintf(
			...
			esc_html__( 'Title', 'shrake' ),
			esc_html__( 'Type', 'shrake' ),
			esc_html__( 'Date', 'shrake' ),
			esc_html__( 'Status', 'shrake' )
		);

		foreach ( $posts as $post ) {
			$title = trim( $post->post_title ) ? $post->post_title : esc_html__( '(no title)', 'shrake' );
			$alt   = ( 'alternate' === $alt ) ? '' : 'alternate';

			switch ( $post->post_status ) {
				case 'publish' :
				case 'private' :
					$stat = esc_html__( 'Published', 'shrake' );
					break;
				case 'future' :
					$stat = esc_html__( 'Scheduled', 'shrake' );
					break;
				case 'pending' :
					$stat = esc_html__( 'Pending Review', 'shrake' );
					break;
				case 'draft' :
					$stat = esc_html__( 'Draft', 'shrake' );
					break;
			}
			...
			if ( '0000-00-00 00:00:00' === $post->post_date ) {
				...
				$time = mysql2date( esc_html__( 'Y/m/d', 'shrake' ), $post->post_date );
			}
