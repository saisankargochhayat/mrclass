...
class PhpThumbHelper extends HtmlHelper {
    ...
    private function get_thumb_data() {
        ...
        if ($this->error) {
            ...
            $this->thumb_data['src'] = trim(HTTP_ROOT, '\\/') . $this->options['display_path'] . '/' . substr($this->cache_filename, strrpos($this->cache_filename, DS) + 1, strlen($this->cache_filename));
        }
    ...
    private function validate() {
        if (!is_file($this->options['src'])) {
            ...
            $this->options['src'] = WWW_ROOT . "images" . DS . "noimage.png";
            return;
        }
    ...
    function generateThumbnail($image, $options) {
        ...
        if ($options['w'] == '' && $options['h'] == '' && file_exists($options['src'])) {
            $image_size = getimagesize($options['src']);
            $options['w'] = $image_size[0];
            $options['h'] = $image_size[1];
            $options['zc'] = '0';
        }
        if ($options['w'] != '' && $options['h'] != '' && $options['zc'] == 2 && file_exists($options['src'])) {
            $image_size = getimagesize($options['src']);
            $options['w'] = $options['w'] < $image_size[0] ? $options['w'] : $image_size[0];
            $options['h'] = $options['h'] < $image_size[1] ? $options['h'] : $image_size[1];
            $options['zc'] = '0';
        }
