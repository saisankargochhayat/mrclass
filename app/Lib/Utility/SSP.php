...
class SSP {
     ...
     *
     */
    static function simple($request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $customOrder = NULL, $having = '') {
        $bindings = array();
        ...
        if (empty($having)) {
            ...
            $where = "";
            $having = SSP::filterHaving($request, $columns, $bindings, $joinQuery);
        }
        ...
        if ($customOrder)
            $order = ($order) ? $order . ', ' . $customOrder : $order;
        ...
        if (strpos($query, '`DATE_FORMAT(`created`,"%b %d,%Y")`') !== false) {
            $query = str_replace('`DATE_FORMAT(`created`,"%b %d,%Y")`', 'DATE_FORMAT(`created`,"%b %d,%Y")', $query);
        }
        if (strpos($query, '`DATE_FORMAT(`created`,"%b %d,%Y %I:%i %p")`') !== false) {
            $query = str_replace('`DATE_FORMAT(`created`,"%b %d,%Y %I:%i %p")`', 'DATE_FORMAT(`created`,"%b %d,%Y %I:%i %p")', $query);
        }
    ...
    ##############################################################################################
    #######Custom Functions Needed for Application(Will work from view,model & controllers)#######
    ##############################################################################################
    ...
    static function get_subscription($user_id) {
        APP::import('Model', 'Subscription');
        $Subscription = new Subscription();
        $is_subscribed = $Subscription->find('first', array('conditions' => array('Subscription.user_id' => $user_id), 'order' => array('Subscription.created' => 'DESC'), 'fields' => array('Subscription.*', 'DATE_FORMAT(Subscription.subscription_start,"%Y-%m-%d") as subscription_start', 'DATE_FORMAT(Subscription.subscription_end,"%Y-%m-%d") as subscription_end', 'User.*')));
        return $is_subscribed;
    }

    static function filterHaving($request, $columns, &$bindings, $isJoin = false) {
        $globalSearch = array();
        $columnSearch = array();
        $dtColumns = SSP::pluck($columns, 'dt');
        ...
        if (isset($request['search']) && $request['search']['value'] != '') {
            ...
            for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
                ...
                if ($requestColumn['searchable'] == 'true') {
                    $binding = SSP::bind($bindings, '%' . $str . '%', PDO::PARAM_STR);
                    $globalSearch[] = ($isJoin) ? $column['as'] . " LIKE " . $binding : "`" . $column['as'] . "` LIKE " . $binding;
                }
            }
        }
        ...
        for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
            ...
            if ($requestColumn['searchable'] == 'true' &&
                    $str != '') {
                $binding = SSP::bind($bindings, '%' . $str . '%', PDO::PARAM_STR);
                $columnSearch[] = ($isJoin) ? $column['as'] . " LIKE " . $binding : "`" . $column['as'] . "` LIKE " . $binding;
            }
        }
