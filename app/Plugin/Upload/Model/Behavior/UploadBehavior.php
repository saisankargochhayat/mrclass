...
class UploadBehavior extends ModelBehavior {
    ...
    public $defaults = array(
        ...
        'path' => '{ROOT}webroot{DS}upload{DS}{model}{DS}{field}{DS}',
    ...
    public function beforeSave(Model $model, $options = array()) {
        ...
        foreach ($this->settings[$model->alias] as $field => $options) {
            ...
            /* GKM */
            $this->runtime[$model->alias][$field]['name'] = (microtime(true) * 10000) . "_" . $this->runtime[$model->alias][$field]['name'];
            $model->data[$model->alias] = array_merge($model->data[$model->alias], array(
    ...
    public function afterSave(Model $model, $created, $options = array()) {
        ...
        foreach ($this->settings[$model->alias] as $field => $options) {
            ...
            /* GKM */
            if (isset($model->data[$model->alias][$options['fields']['dir']]) && trim($model->data[$model->alias][$options['fields']['dir']]) != '') {
                $this->__filesToRemove[$model->alias][] = @dirname($filePath) . DS . $model->data[$model->alias][$options['fields']['dir']];
            }
    ...
    protected function _path(Model $model, $field, $options = array()) {
        ...
        $newPath = Folder::slashTerm(str_replace(
                                array_keys($replacements), array_values($replacements), $newPath
        ));
